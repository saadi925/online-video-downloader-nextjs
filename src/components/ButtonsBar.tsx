    ...
    <div className='flex w-full flex-wrap justify-center py-2 items-center  rounded-t-xl  '>
      {
        buttons.map((btn : any, index)=>(
            <AppButton key={index} Icon={btn.Icon} isActive={isBtnActive(btn)} click={()=> onChange(btn.title.toLowerCase())} className={`md:first:rounded-l-2xl  md:last:rounded-r-2xl bg-white !py-1 hover:bg-paper-black hover:text-white `} title={btn.title}/>
        ))
      }
    </div>
  )
